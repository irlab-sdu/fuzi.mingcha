...
tokenizer = AutoTokenizer.from_pretrained(
    "SDUIRLab/fuzi.mingcha-v1.0", trust_remote_code=True
)
model = (
    AutoModel.from_pretrained("SDUIRLab/fuzi.mingcha-v1.0", trust_remote_code=True)
    .half()
    .cuda()
)
model = model.eval()
...
def main():
    ...
    while True:
        ...
        if mod == 0:  # 欢迎页
            ...
            data = {"query": process_lucence_input(generate_law), "top_k": 2}
            ...
            response, _ = chat(
                prompt2_task1.replace("@检索得到的相关法条@", retrieval_law).replace(
                    "@用户输入@", query
                )
            )
            print(f"\n\n夫子·明察·法条检索：\n{response}")
            ...
            for i, doc in enumerate(docs):
                retrieval_law = retrieval_law + f"第{i + 1}条：\n{doc}\n"
            response, _ = chat(
                prompt2_task2.replace("@检索得到的相关案例@", retrieval_law).replace(
                    "@用户输入@", query
                )
            )
            print(f"\n\n夫子·明察·类案检索：\n{response}")
